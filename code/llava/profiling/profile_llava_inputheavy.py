"""LLaVA-1.5-7B input-heavy workload (mirrors mllm_sm_vars VQA spec): 128 text
input tokens, 16 output tokens. LLaVA-1.5 has a fixed 336x336 image size — it
does not dynamic-tile — so the input-heavy / output-heavy distinction here
manifests purely as output token count, with input held constant. Run from a
directory containing eer.jpg.
...
INPUT_TOKENS = 128
OUTPUT_TOKENS = 16
IMAGE_SIZE = 336  # LLaVA-1.5 native; resizing to anything else just gets downsampled
