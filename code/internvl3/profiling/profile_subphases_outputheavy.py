"""InternVL3 output-heavy workload (matches mllm_sm_vars Generation): 448x448
image, 128 text input tokens, 256 output tokens. Run from a directory
containing eer.jpg.
...
llm = LLM(
    ...
    max_model_len=2048,                # 448x448 single tile (~256 image tokens) + 128 text + 256 out fits comfortably
...
# Distinct images per request defeat vLLM's encoder cache so each
# profiled forward actually re-runs the vision tower.
